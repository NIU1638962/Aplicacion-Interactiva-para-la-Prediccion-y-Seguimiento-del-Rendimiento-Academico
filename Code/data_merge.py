...
def merge_data():
    ...
    subfolders = tuple([
        file.path for file in environment.os.scandir(
    ...
    for subfolder in subfolders[:3]:
        ...
        utils.print_message(
            ...
            + f'\nShape: [{dataframe.shape[0]} rows'
            + f' x {dataframe.shape[1]} columns]\n'
            + 'Memory used: '
            + f'{utils.transform_redable_byte_scale(dataframe.memory_usage(deep=True).sum())}'
        )

    dataframe = dataframe.astype(environment.numpy.float32)
    ...
    utils.print_message(
        f'There is "{len(dataframe["target"].unique())}" appearences of'
        + f' unique grades out of "{environment.NUMBER_GRADES}" expected in'
        + ' the range.'
    )

    utils.print_message(
        'Eachs grade has the following percentage of appearances:\n'
        + str(
            (
                (
                    (
                        dataframe["target"].value_counts().sort_index(
                            ascending=False,
                        )
                    ) / (
